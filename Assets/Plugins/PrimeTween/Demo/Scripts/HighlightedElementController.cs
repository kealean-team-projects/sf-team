...
namespace PrimeTweenDemo {
    public class HighlightedElementController : MonoBehaviour {
        ...
        private void Update() {
            if (cameraProjectionMatrixAnimation.IsAnimating) return;
            ...
            if (!new Rect(0f, 0f, Screen.width, Screen.height).Contains(screenPosition)) return;
            var ray = mainCamera.ScreenPointToRay(screenPosition);
            var highlightableElement = RaycastHighlightableElement(ray);
            SetCurrentHighlighted(highlightableElement);

            if (current != null && InputController.GetDown()) current.GetComponent<Animatable>().OnClick();
        }
        ...
        private void SetCurrentHighlighted([CanBeNull] HighlightableElement newHighlighted) {
            if (newHighlighted != current) {
                if (current != null) AnimateHighlightedElement(current, false);
                current = newHighlighted;
                if (newHighlighted != null) AnimateHighlightedElement(newHighlighted, true);
            }
        }
        ...
        private static void
            AnimateHighlightedElement([NotNull] HighlightableElement highlightable, bool isHighlighted) {
            Tween.LocalPositionZ(highlightable.highlightAnchor, isHighlighted ? 0.08f : 0, 0.3f);
            foreach (var model in highlightable.models)
                Tween.MaterialColor(model.sharedMaterial, emissionColorPropId,
                    isHighlighted ? Color.white * 0.25f : Color.black, 0.2f, Ease.OutQuad);
        }
    }
}
#endif
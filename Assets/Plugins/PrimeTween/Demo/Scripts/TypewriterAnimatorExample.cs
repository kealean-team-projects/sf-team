...
namespace PrimeTweenDemo {
    ...
    public class TypewriterAnimatorExample : MonoBehaviour {
        ...
        /// <summary>
        ///     A simple typewriter animation that uses built-in <see cref="Tween.TextMaxVisibleCharacters()" /> animation
        ///     method.
        /// </summary>
        public Tween TypewriterAnimationSimple() {
            text.ForceMeshUpdate();
            var characterCount = text.textInfo.characterCount;
            var duration = characterCount / charsPerSecond;
        ...
        public Tween TypewriterAnimationWithPunctuations() {
            text.ForceMeshUpdate();
            RemapWithPunctuations(text, int.MaxValue, out var remappedCount, out _);
            var duration = remappedCount / charsPerSecond;
            return Tween.Custom(this, 0f, remappedCount, duration, (t, x) => t.UpdateMaxVisibleCharsWithPunctuation(x),
                Ease.Linear);
        }

        private void UpdateMaxVisibleCharsWithPunctuation(float progress) {
            var remappedEndIndex = Mathf.RoundToInt(progress);
            RemapWithPunctuations(text, remappedEndIndex, out _, out var visibleCharsCount);
            if (text.maxVisibleCharacters != visibleCharsCount) text.maxVisibleCharacters = visibleCharsCount;
            // play keyboard typing sound here if needed
        }

        private void RemapWithPunctuations([NotNull] TMP_Text text, int remappedEndIndex, out int remappedCount,
            ...
            for (var i = 0; i < count; i++) {
                if (remappedCount >= remappedEndIndex) break;
                remappedCount++;
                visibleCharsCount++;
        ...
        public Tween TypewriterAnimationByWords() {
            text.ForceMeshUpdate();
            RemapWords(text, int.MaxValue, out var numWords, out _);
            var duration = text.textInfo.characterCount / charsPerSecond;
        ...
        private void UpdateVisibleWords(float progress) {
            var curWordIndex = Mathf.RoundToInt(progress);
            RemapWords(text, curWordIndex, out _, out var visibleCharsCount);
            if (text.maxVisibleCharacters != visibleCharsCount) text.maxVisibleCharacters = visibleCharsCount;
            // play keyboard typing sound here if needed
        }

        private static void RemapWords([NotNull] TMP_Text text, int remappedEndIndex, out int remappedCount,
            out int visibleCharsCount) {
            visibleCharsCount = 0;
            var count = text.textInfo.characterCount;
            if (count == 0) {
            ...
            var characterInfos = text.textInfo.characterInfo;
            for (var i = 0; i < count; i++) {
                if (remappedCount >= remappedEndIndex) return;
                remappedCount++;
